...
class HARCNN(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1).unsqueeze(2)  # add height dim
        out = self.conv1(x)
        out = self.conv2(out)
...
class EEGCNN(nn.Module):
    def __init__(self, configs):
        ...
        self.fc = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
    ...
    def forward(self, x_in):
        ...
        return x_out  # Directly returning class scores (logits)
