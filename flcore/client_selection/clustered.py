...
class ClusteredSampling1(ClientSelection):
    ...
    def setup(self, n_samples):
        ...
        #distri_clusters = np.zeros((self.n_cluster, self.total)).astype(int) ##NOUR: removed and replaced by following line to prevent overflow
        distri_clusters = np.zeros((self.n_cluster, self.total)).astype(np.int64)
        k = 0
        ...
        for l in range(self.n_cluster):
            ##NOUR: i think the following is causing errors
            #distri_clusters[l] /= np.sum(distri_clusters[l])
            ##NOUR: new code starts here
            cluster_sum = np.sum(distri_clusters[l])
            if cluster_sum == 0:
                ##NOUR: If a cluster has no probability mass, distribute it uniformly among all clients.
                ##NOUR: This prevents division by zero in the next step.
                print(f"WARNING: Cluster {l} in ClusteredSampling1.setup has sum=0. Distributing uniformly.")
                distri_clusters[l] = np.ones(self.total) / self.total
            else:
                distri_clusters[l] /= cluster_sum
            ##NOUR: new code ends here
    ...
    def select(self, n, client_idxs, metric=None):
        ...
        for k in range(n):
            ...
            ##NOUR: also removing the next line to handle 0 sum cases
            #selected_client_idxs.append(int(np.random.choice(client_idxs, 1, p=weight/sum(weight))))
            ##NOUR: new code starts here
            weight_sum = np.sum(weight) ##NOUR: Calculate sum separately

            if weight_sum == 0:
                ##NOUR: If the selected cluster portion has no weight, choose uniformly from available clients (client_idxs)
                print(f"WARNING: Sum of weights for selection in cluster {k} in ClusteredSampling1.select is zero. Selecting uniformly from available client_idxs.")
                selected_client_idxs.append(int(np.random.choice(client_idxs, 1))) # np.random.choice without 'p' uses uniform
            else:
                selected_client_idxs.append(int(np.random.choice(client_idxs, 1, p=weight/weight_sum)))
            ##NOUR: new code ends here
...
class ClusteredSampling2(ClientSelection):
    ...
    def get_clusters_with_alg2(self, linkage_matrix: np.array, n_sampled: int, weights: np.array):
        ...
        #pop_clusters = np.zeros((n_clusters, 2)).astype(int) ##NOUR: removed and replaced with next line to prevent overflow
        pop_clusters = np.zeros((n_clusters, 2)).astype(np.int64)
        for i in range(n_clusters):
            pop_clusters[i, 0] = i + 1
            for client in np.where(clusters == i + 1)[0]:
                #print("step reached, value being added is ",weights[client] * epsilon * n_sampled) ##NOUR: overflow error here, trying to debug
                #pop_clusters[i, 1] += int(weights[client] * epsilon * n_sampled) ##NOUR: line removed and hanged to next line to do casting implicitely and avoid overflow.
                pop_clusters[i, 1] += (weights[client] * epsilon * n_sampled)
        pop_clusters = pop_clusters[pop_clusters[:, 1].argsort()]

        #distri_clusters = np.zeros((n_sampled, n_clients)).astype(int) ##NOUR: also removed and replaced by next line for overflowing reasons
        ...
        for idx, cluster in enumerate(kept_clusters):
            for client in np.where(clusters == cluster)[0]:
                #distri_clusters[idx, client] = int(
                    #weights[client] * n_sampled * epsilon) ##NOUR: removed and replaced, implicit casting for overflow
                distri_clusters[idx, client] = (
        ...
        for j in pop_clusters[: n_clusters - n_sampled, 0]:
            ...
            for client in clients_in_j:
                #weight_client = int(weights[client] * epsilon * n_sampled) ##NOUR: removed and replaced for overflow prevention
                weight_client = (weights[client] * epsilon * n_sampled)
